...
public class D {
	...
	public static int sol(double[] A, double[] B, int N) {	
		int points = 0;
		int chosen_b = 0;
		for(int i=0; i<N; i++) {			
			while(chosen_b<N  && B[chosen_b]<A[i]) {
				chosen_b++;
			}
			if(chosen_b < N) {				
				points++;
	...
	public static String solution (double[] naomi, double[] ken, int N) {
		qsort(naomi);
		qsort(ken);
		
		int no_cheat = N - sol(naomi, ken, N);
		int cheat = sol(ken, naomi, N);
		
		return Integer.toString(cheat)+" "+Integer.toString(no_cheat);
	}
	
	public static void main(String[] args) throws IOException {
		String dir = System.getProperty("user.dir")+"\\src\\round\\qualification\\";
//		Scanner reader = new Scanner(new FileReader(dir+"D-small-attempt0.in"));
//		PrintWriter writer = new PrintWriter(new FileWriter(dir+"D-small-attempt0.out"));
		Scanner reader = new Scanner(new FileReader(dir+"D-large.in"));
		PrintWriter writer = new PrintWriter(new FileWriter(dir+"D-large.out"));
		...
		for(int i=1; i<=numOfCases; i++) {
			...
			for(int j=0; j<N; j++) {
				naomi[j] = reader.nextDouble();
			}
			double[] ken = new double[N];
			for(int j=0; j<N; j++) {
				ken[j] = reader.nextDouble();
			}
			
			String answer = solution(naomi, ken, N);	
